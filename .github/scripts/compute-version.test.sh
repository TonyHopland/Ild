...
expect_ok() {
  ...
  local actual
  if ! actual="$("$script" "$@" "$props" 2>/dev/null)"; then
...
expect_fail() {
  local desc="$1"; shift
  if "$script" "$@" "$props" >/dev/null 2>&1; then
...
expect_ok "release tag v1.2.3"   "1.2.3"   tag    v1.2.3   abc1234
expect_ok "release tag v0.2.0"   "0.2.0"   tag    v0.2.0   deadbee
expect_ok "prerelease tag"       "2.0.0-rc.1" tag v2.0.0-rc.1 abc1234

# main builds stamp <base>-main+<shortsha> from props, leaving assembly numbers clean.
expect_ok "main branch build"    "9.9.9-main+abc1234" branch main abc1234

# Edge cases: the stray bare tag and other non-canonical refs must be rejected.
expect_fail "bare tag 0.2.0 (no v prefix)" tag 0.2.0 abc1234
expect_fail "non-semver tag"               tag vnope abc1234
expect_fail "unsupported ref type"         release v1.2.3 abc1234
