...
- **GitHub Copilot AI provider** — a new `copilot` provider type backed by a `CopilotAdapter` that runs GitHub's `copilot` CLI headlessly (`--prompt --allow-all-tools --add-dir`) inside the run's worktree. Like `claude-code` it is a CLI-auth managed agent: it installs on demand from the `@github/copilot` npm package onto `/data`, authenticates via the CLI's own `/login` (a GitHub Copilot subscription, credentials under `~/.copilot`), and needs no Base URL, API key, or model. The AI Provider page hides those fields and shows a Copilot-specific login note when the type is selected.
