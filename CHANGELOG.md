...
- **CI now builds the container images on every PR** (`image-build`), including the `WITH_DOTNET_SDK=0` base that no other build selects; arm64 is built on PRs that touch the Dockerfiles (`image-arm64`) and via manual dispatch. Previously the Dockerfiles were only ever built by `publish.yml` at release time, so a Dockerfile regression failed the release tag rather than the change that introduced it. Neither job pushes.
