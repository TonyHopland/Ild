...
public static class AgentRunFiles
{
    ...
    /// legacy <c>extensions/ild.ts</c>. Returns whether the legacy files are all gone.
    /// </summary>
    public static Task<bool> SweepAtStartupAsync(IReadOnlySet<Guid> activeRunAndChatIds, CancellationToken ct = default)
        => SweepAtStartupAsync(
            activeRunAndChatIds,
    ...
    /// <inheritdoc cref="SweepAtStartupAsync(IReadOnlySet{Guid}, CancellationToken)"/>
    internal static Task<bool> SweepAtStartupAsync(
        IReadOnlySet<Guid> activeRunAndChatIds, string agentReadRoot, string scratchRoot, DateTime startedUtc, CancellationToken ct)
    {
        IldMcpServer.SweepConfigFiles(agentReadRoot, startedUtc);
        PiAdapter.SweepExtensions(agentReadRoot, activeRunAndChatIds);
        return PiAdapter.SweepLegacyExtensionsAsync(scratchRoot, ct);
    }
}
