...
public sealed class ChatService : IChatService
{
    ...
    private const string LoopAuthoringGuide =
        ...
        - Anchor matchRules on a verdict you instruct the agent to emit last, but anchor it with (?m)^TO_REVIEW$ — a bare ^TO_REVIEW$ requires the verdict to be the ENTIRE output, so an agent that explains itself first never matches it and falls through to OnSuccess instead. An unanchored token works too; it is just beatable by an earlier mention.
