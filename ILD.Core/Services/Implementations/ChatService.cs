...
public sealed class ChatService : IChatService
{
    ...
    private const string LoopAuthoringGuide =
        ...
        - AI: runs the agent. config.prompt, config.aiProviderId, config.toolAllowlist (string[]), config.matchRules ([{ "pattern", "edgeName" }] routing the AI output to Custom edges by name — patterns are case-insensitive regexes and the rule matching LATEST in the output wins, so an agent's closing verdict beats a word it mentioned earlier; no match takes OnSuccess). A pattern must be a valid regex and must require at least one character: zero-width patterns (x*, \b, (?:), (?=...)) match at the end of every output and would always win, so they are rejected.
