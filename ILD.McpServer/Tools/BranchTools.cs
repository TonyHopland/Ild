...
public sealed class BranchTools
{
    ...
    [Description("Pull the latest changes from origin into this work item's run branch — fetches origin with ILD's repository credentials and rebases the worktree branch onto its own remote counterpart (origin/<branch>). Use this to pick up commits pushed to the branch after the run started; you cannot do it yourself, git in the worktree has no credentials. The fetch covers every branch on the remote, not just yours, so afterwards you can read any of them locally with plain git — origin/<default branch> included, which is how you check whether your branch has fallen behind it and merge it in yourself if it has. What this tool will not do for you is that merge: it only rebases your branch onto its own remote counterpart. Returns an 'outcome' of Updated, AlreadyUpToDate (nothing new on origin), NoRemoteBranch (the branch was never pushed — nothing to pull, not an error), DirtyWorktree (commit your changes first; 'files' lists them), Conflict (the rebase was aborted and the branch left untouched; 'files' lists the conflicted paths to resolve) or RebaseRefused (git would not rebase at all — nothing to resolve, read 'message').")]
