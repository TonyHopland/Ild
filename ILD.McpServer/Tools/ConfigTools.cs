...
public sealed class ConfigTools
{
    ...
    internal const string SchemaDocument = """
    {
      ...
      "howItWorks": [
        ...
        "When the ILD instance is configured with a preview proxy base, services are reached on wildcard subdomains — wi-<workItemId> for the 'public' service, wi-<workItemId>-<serviceName> for any other — and the allocated port is never exposed directly. ILD rewrites the Host header to the loopback target so host-checking dev servers work unchanged; set 'rewriteHost': false on a service that must see the browser-facing hostname."
      ],
      "templateTokens": {
