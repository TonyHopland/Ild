...
**AI Node**:
Delegates execution to an `IAgentAdapter` resolved by `AiProvider.Type`. The adapter controls the full execution lifecycle including multi-turn loops, tool use, and internal state. Has a single `prompt` field regardless of whether the node starts fresh or resumes a bound session. If the graph needs different first-turn and follow-up prompts, model that explicitly with upstream `Prompt` nodes. Provider resolution: if `aiProviderId` parses as a GUID, the executor looks it up and fails if the row is missing (no fallback). If `aiProviderId` is unset or not a GUID, the executor falls back to `GetDefaultAiProviderAsync` (the provider flagged as default), failing only if no default is configured. The resolved provider can then be overridden by the **WorkItem's AI provider override** (`WorkItem.AiProviderOverride` + `AiProviderOverrideId`): `OverrideAll` swaps every AI node's provider, `OverrideDefault` swaps only nodes that fell back to the default (a node pinned to a specific provider is left alone), and `None` (the default) leaves the node's own choice intact; an override whose target provider is missing fails the node. Supports `rejectPattern` — a regex that, when matched against the AI output, causes the node to fail and route to the `on_failure` edge. Bounded by run-scoped cancellation only — no per-node timeout.
