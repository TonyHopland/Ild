...
The work items this ILD instance is currently working on: one per local `LoopRun` still alive — `Running`, or `WaitingHuman`, which covers both a run parked at a Human/PR gate and one halted mid-node by a human (`LoopRun.IsHalted`). `RemoteWorkItemCoordinator` **derives** it from `ILoopRunStore.GetActiveRunsAsync()` at the top of every poll pass — it is never maintained incrementally, so a run that ended releases its work item whatever status the item itself landed in. The same set does both jobs: it is the heartbeat list the server's stale reclaimer keys off, and its size is the concurrency gate against `scheduler.maxConcurrent` (claims made during a pass are counted locally on top, so a run that both starts and ends within one pass cannot hand its slot to a second claim in that pass). The cap is therefore **per instance** — deriving it from the server's `Running` status instead would silently make it global across every instance pointed at the same board. A pass that has Ready work but no room reports it as blocked along with the ids holding the slots; `WorkItemScheduler` logs that on entering the state and on every change of holders, since being at the cap persists across passes and grace polling would otherwise repeat it every 5s.
