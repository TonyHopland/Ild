...
**PR History**:
Every pull request ever opened against a WorkItem, surfaced on `WorkItemView.PullRequests` (`pullRequests` on the API) newest run first and deduplicated by URL. ADR-0008 gives each LoopRun at most one PR, so a retried item accumulates one entry per run; an entry is self-contained (url, merged, last badge status, when it was first seen, and the run that opened it while that run still exists) because the run it came from is on its way to being reclaimed. Assembled from two sources that never overlap for long: the PRs on the item's live runs, plus a `WorkItemPullRequestRecord` per PR whose run has been deleted — written by `ILoopRunStore.DeleteAsync` at the moment of deletion, exactly like the analytics rollup, so nothing has to be kept in sync while the run lives and neither the retention sweeper nor a manual run delete can take the link away. Dropped only when the WorkItem itself is deleted. Distinct from `prUrl`, which keeps its narrower meaning — the **current** run's PR, and therefore empty once that run goes terminal (`/api/v1` is add-only per [ADR-0002](docs/adr/0002-manual-api-versioning.md), so `prUrl` was left alone rather than widened).
...
- A **Worktree** (and its branch) is created per **LoopRun** by the Start node and kept after the run finishes for inspection; the `WorktreeRetentionSweeper` reclaims worktree, branch, and run once terminal longer than `run.retentionDays` (settings-driven; `0` disables; runs marked `Retain` are never reclaimed). What outlives the row is archived as it goes: the run's metrics into the analytics rollup, its PR into the work item's **PR History**. See [ADR-0008](docs/adr/0008-worktree-and-branch-per-run.md)
