...
**Worktree**:
A per-**run** git worktree on a per-run branch (`ild/wi-<workItemId>-run-<runId>`), created by the Start node. It is **kept** after the run finishes so the run stays inspectable, and three paths destroy it — a manual delete (`DELETE /api/v1/loopruns/{id}`, or deleting the whole work item), the `WorktreeRetentionSweeper` once the run has been terminal longer than `run.retentionDays`, and a **Run Cleanup**. All three go through the shared `IRunReclaimer`, which stops any **Worktree Preview** on the worktree before destroying it (best-effort — a preview is addressed by worktree path, so it would otherwise be left holding its ports unreachably), and reclamation is verified: nothing is written until worktree and branch are confirmed gone, otherwise the run is left as it was (a manual delete or cleanup returns 409) so a later sweep retries. Sending a work item to Done/Backlog finishes the run but does **not** touch its worktree or branch. See [ADR-0008](docs/adr/0008-worktree-and-branch-per-run.md).
...
**Start Node**:
The entry point of a loop graph. Optionally creates a worktree and branch. If the base git repository is missing at `repo.WorktreesPath` (or fallback `{DataRoot}/repos/{repo-id}`), it is cloned on demand. If the base repository already exists, `git fetch origin` is run followed by `git reset --hard origin/<base>`. Both fetch failure and reset failure **fail the node**, enforcing the run-isolation invariant that every run must start from the latest origin base — a swallowed fetch failure would otherwise reset/rebase against a stale local `origin/<base>` and silently start the run on outdated code. After worktree creation, the branch is rebased onto `origin/<base>` — rebase failure also fails the node to prevent stale worktrees. `<base>` is the work item's **base branch override** if it has one, otherwise the repository's default branch; the override is pinned on the `LoopRun` at creation and reaches all three of reset, worktree, and rebase. An override that names a branch `origin` does not have fails the node with that reason rather than falling back to the default — see [ADR-0008](docs/adr/0008-worktree-and-branch-per-run.md).
_Avoid_: init, setup
...
**AI Node**:
Delegates execution to an `IAgentAdapter` resolved by `AiProvider.Type`. The adapter controls the full execution lifecycle including multi-turn loops, tool use, and internal state. Has a single `prompt` field regardless of whether the node starts fresh or resumes a bound session. If the graph needs different first-turn and follow-up prompts, model that explicitly with upstream `Prompt` nodes. Provider resolution: if `aiProviderId` parses as a GUID, the executor looks it up and fails if the row is missing (no fallback). If `aiProviderId` is unset or not a GUID, the executor falls back to `GetDefaultAiProviderAsync` (the provider flagged as default), failing only if no default is configured. The resolved provider can then be overridden by the **WorkItem's AI provider override** (`WorkItem.AiProviderOverride` + `AiProviderOverrideId`): `OverrideAll` swaps every AI node's provider, `OverrideDefault` swaps only nodes that fell back to the default (a node pinned to a specific provider is left alone), and `None` (the default) leaves the node's own choice intact; an override whose target provider is missing fails the node. Supports `rejectPattern` — a regex that, when matched against the AI output, causes the node to fail and route to the `on_failure` edge. Bounded by run-scoped cancellation only — no per-node timeout. **Session naming** (`sessionPlaceholder`, `forkFromPlaceholder`) is templated, but on a deliberately narrower grammar than a prompt: `{{Var.<name>}}` only, with every other placeholder family rejected at save time (`{{PreviousNode.Output}}` as a session name would mint an unbounded new session every turn). This makes a session per-iteration rather than per-loop: `ticket_{{Var.current_ticket}}` on a node inside a cycle holds the conversation _within_ one item and starts fresh on the next, resuming whenever the same value recurs. Both fields resolve **once** per execution, before the provider slot is claimed, and the one resolved value feeds both the binding lookup and the bind — resolving twice could resume one session and record another. Resolution is strict where the prompt pipeline is lenient: an unset variable, an empty result, or one over the binding store's 128-character key limit **fails the node**, because an empty resolve would silently drop every iteration back into one shared session. A literal name behaves exactly as before and never reads the variable store.
...
**PR Node**:
Creates a pull request (or reuses the one already linked on the run via `LoopRun.PrUrl` — PRs are per-run, see [ADR-0008](docs/adr/0008-worktree-and-branch-per-run.md)). Before PR creation, automatically commits any uncommitted changes, pushes the branch, fetches the remote, and verifies the branch has commits ahead of the target branch — if the branch has 0 commits ahead, the node fails (no-changes guard). The target is the same base the Start node built the run from — the work item's base branch override if it has one, otherwise the repository's default branch — read from the run's pinned copy, so a PR always goes back to the branch its worktree was rebased onto. Supports three template config fields, all rendered through the prompt placeholder pipeline: `prompt` (announced as the node's input), `prDescriptionTemplate` (used as the PR body on creation; falls back to `WorkItem.Description` if unset), and `prCommentTemplate` (posted as a fresh PR comment on every re-visit when the PR already exists; the node fails if the comment post fails). When the work item carries the `AutoMerge` tag (matched case-insensitively), the node turns on the provider's auto-merge for the PR right after creating it — GitHub via the `enablePullRequestAutoMerge` GraphQL mutation, Forgejo/Gitea via the merge endpoint's `merge_when_checks_succeed`, Azure DevOps via the pull request's auto-complete flag armed as the token's own identity. This is best-effort: a repository or provider that does not support auto-merge is a silent no-op, never a node failure, leaving the PR for the heartbeat/human merge path. Makes the PR lifecycle explicit in the graph.

**PR Heartbeat**:
While a PR node is parked in `PrAwaitingMerge` (`WaitingHuman` with a `PrUrl`), the `PrStatusPoller` background service polls the provider every `pr.heartbeatSeconds` (global app setting; UI dial in Settings), fetches a full `RemotePrSnapshot` (title, description, mergeability, CI verdict, the failing checks behind a red verdict — name, conclusion, link and the provider's own truncated check output — review decision, full conversation), persists it on the `LoopRun` (driving the feedback UI's full PR view, pushed live over `LoopRunHub`'s `PrSnapshotChanged`), and diffs it against the previous tick to fire **named custom edges on state transitions**. The seven reserved edges, in priority order, are: `on_rejected` (a review requested changes) › `on_merge_conflict` (`mergeable=false`/`mergeable_state=dirty`) › `on_ci_failed` (a check failed/timed-out/cancelled, or a commit status is failure/error) › `on_approved` (an approving review, not since dismissed) › `on_ci_passed` (all checks succeeded) › `on_merged` (closed **with** merge) › `on_abandoned` (closed **without** merge). Each tick the poller emits a `NodeSignal.Custom` for the single highest-priority edge that is both **newly true this tick** and **actually connected** on the node — an unconnected reserved edge never fires and never fails the run; it only refreshes the snapshot/GUI. The transition baseline is empty at park time and resets every re-park, so a state already true when the run parks (or still true after a fix loop returns to the node) fires on the first poll. There is **no fallback** to `on_success`/`on_failure` for any state — in particular a merged or abandoned PR leaves the run parked indefinitely unless `on_merged`/`on_abandoned` are wired, so templates **must** wire them to reach a terminal/Cleanup path. Every emitted signal carries a **reason** as its output — the edge's meaning in prose plus the detail behind it (the failing checks for `on_ci_failed`, the review that asked for changes for `on_rejected`) — so the resumed PR node's output, i.e. the next node's `{{PreviousNode.Output}}`, says why it moved on instead of arriving empty; the text is capped at 8192 characters. The webhook path (`PrSyncService`) maps to the same `on_*` edges, is likewise connected-gated, and sends the same reason (preferring the webhook's own review comment when it has one).
_Avoid_: repository node, git node

**CI Log Handle**:
A failing check's summary is what the provider chose to say, not the error, so each entry in a snapshot's failing checks carries a **check id** and the `on_ci_failed` reason spells out the `get_ci_log` call that reads the log behind it — an agent cannot follow a `details_url`, and has no placeholder for its own work item id. The tool (`get_ci_log(workItemId, checkId, tailLines, offset)`, agent API `GET /api/v1/agent/workitems/{id}/ci-log`) fetches server-side with the forge credentials the agent does not hold, and only for a check id that appears in that work item's own last snapshot. It answers with the **tail** — the error is at the end — capped per call at 16k characters and flagged `truncated` so the agent pages with `offset` rather than assuming it saw everything. Logs are never inlined into a reason or a snapshot. On GitHub the id is the Actions **job** id (taken from `details_url`, not the check-run id) and the log is one authenticated call away; on Azure DevOps it is the **build** id behind the failing branch policy, and what comes back is the log of the timeline step that failed rather than the whole build. Where CI lives outside the forge (Forgejo commit statuses published by Woodpecker et al., an Azure DevOps PR status posted by an external system) there is nothing to fetch, and the tool says so with the check's URL rather than failing.
_Avoid_: CI artifacts, log streaming

**Condition Node**:
A switch over run/work-item state: an ordered list of predicate **cases**, each routing to a named custom edge, plus a **default edge** taken when no case matches — all without invoking AI, running a command, or touching the worktree. Each case's **Condition Variant** picks its predicate: `TextMatches` renders the case's `Subject` (default `{{Node.Input}}`) and tests it against `Pattern` with the same case-insensitive `Regex.IsMatch` engine as AI MatchRules; `PrExists` is true iff `LoopRun.PrUrl` is set (local, per-run — no remote call, see [ADR-0008](docs/adr/0008-worktree-and-branch-per-run.md)); `HasTag` fetches the work item fresh and is true iff its `Tags` contain `Tag` by case-insensitive whole-string equality. Cases are evaluated in order and the first whose predicate holds routes to its `EdgeName` custom edge; if none match, the node takes the `DefaultEdge`. The templated `Output` (default `{{Node.Input}}`, a pass-through) is emitted identically on every branch; an evaluation error (e.g. work-item fetch fails, or an uncompilable pattern) fails the node onto `on_failure`. Save-time validation rejects an `OnSuccess` edge, requires a default edge and at least one case, keeps the wired custom edges and the referenced case/default names in sync (like AI MatchRules), and — per `TextMatches` case — rejects a missing or uncompilable `Pattern`. **Legacy migration:** nothing reads the old top-level `Variant`/`Pattern`/`Tag` shape at runtime; the one-time `ConditionSwitchMigrator` rewrites any pre-switch true/false Condition (single case routing to `true`, `false` default — identical routing, existing edges untouched) into the switch shape on startup (idempotent). It is the sole bridge and can be deleted once every deployment has started on this version.
_Avoid_: branch node, if node

**Cleanup Node**:
A terminal (sink) node with only incoming edges. Marks the LoopRun finished. It deliberately **keeps** the worktree and branch so the run stays inspectable — disk is reclaimed only by the `WorktreeRetentionSweeper`, a manual run delete, or a **Run Cleanup**. Despite the name it does the opposite of a Run Cleanup: it ends a run and keeps its worktree. See [ADR-0008](docs/adr/0008-worktree-and-branch-per-run.md).
...
**WaitingForIld**:
WorkItem status used when a node cannot proceed because an internal resource (currently AI provider capacity — ILD's own parallelism gate, before the node runs) is unavailable. The LoopRun stays `Running` from the user's perspective; the scheduler (`RemoteWorkItemCoordinator`) automatically resumes the work item when the resource frees. Distinct from Human Feedback — no human action is required. Set via the `NodeOutcome.WaitingIld(reason)` outcome and paired with `HumanFeedbackReasons.AiProviderThrottled` on the WorkItem. A limit imposed by the **provider** is the other thing entirely and never lands here: it parks in Human Feedback (see **Provider Interruption**), because nothing ILD can wait for will clear it.
_Avoid_: throttled, queued, backpressure

**Provider Interruption**:
An AI node whose adapter reported a failure the provider caused rather than the work failing — usage/session limit reached, HTTP 429, an overloaded provider, a transient mid-stream drop. The adapter classifies it when it has a structured signal (`NodeExecutionResult.Failure`); otherwise `AiFailureClassifier` reads the failure text, output before error, because every CLI adapter renders a non-zero exit as `exit=N stderr=…` and the provider's notice arrives as the agent's output. The two texts get **different rule sets**: the output is also where the agent's own narration lands on a genuine failure, so it is matched only against notices a provider writes and an agent does not (usage/session limits, wire tokens like `rate_limit_error`, an error-framed 429), while ambiguous vocabulary — bare status codes, 5xx phrases, dropped connections — is trusted only from the machine-written error text. The asymmetry deliberately trades recall for precision: an unrecognised interruption keeps its old `on_failure` behaviour, whereas parking a node that genuinely failed would silently break a loop's error handling. The node yields `NodeOutcome.Interrupted`, and the engine parks the run exactly as a Halt does — `WaitingHuman` + `IsHalted`, `HaltReason.Throttled`, node row `Interrupted`, work item in Human Feedback with `HumanFeedbackReasons.AiProviderThrottled` — instead of following `on_failure`. Startup never auto-resumes it (`Throttled` reads like a human halt there, unlike `Shutdown`): the parked run shows the provider's raw output, and the human clicks **Resume** when they judge the limit has reset, continuing the captured agent session — or re-running the node cold if no session had been captured. The `throttle.autoResume` app setting (**off** by default) hands that click to ILD instead: `ThrottledRunResumeSweeper` resumes an eligible park itself, waiting `throttle.retryDelayMinutes` (default 60) between attempts and spending at most `throttle.maxRetries` (default 6) of them, carrying a steering note that marks the resume as automatic. Both are read every sweep, so a change reaches runs already parked. When the notice stated when the limit lifts, that time is a **floor** under the delay and never a trigger: `AiFailureClassifier.TryParseResetAt` reads it at park time onto `LoopRun.ThrottleResetAt` — only a time the provider stamped UTC, or an unambiguous "try again in 20 minutes", never a bare "resets 3pm" whose zone is the reader's — and no attempt fires before it. Otherwise the resume alone **is** the probe: a provider still throttled interrupts the node again so the run re-parks, which is the next attempt, with whatever reset the fresh notice states. An automatic resume is not a human touch, so unlike every other resume it refills neither the **AI Traversal Cap** budget nor its own count (`LoopRun.ThrottleAutoResumeCount`). Both are therefore spent over the same window — since a person last acted, **not** since the current interruption — so a run that auto-resumed earlier and was interrupted again has fewer tries left; once they are gone the run stays parked for a person, exactly where the setting being off would have left it, and one human Resume restores both budgets. Only `Throttled` parks are eligible, never a human's Halt, a `Shutdown` park or a `MaxAiTraversals` park. Genuine failures, **including context-window exhaustion**, still take `on_failure`. See [ADR-0017](docs/adr/0017-shutdown-halts-the-in-flight-ai-node.md).
_Avoid_: rate limit error, provider failure, backoff
...
**PR History**:
Every pull request ever opened against a WorkItem, surfaced on `WorkItemView.PullRequests` (`pullRequests` on the API) newest run first and deduplicated by URL. ADR-0008 gives each LoopRun at most one PR, so a retried item accumulates one entry per run. **A PR is work-item state, not run state** — it touches the repository and outlives the throwaway worktree, branch and PR snapshot that produced it — so it is stored on the **WorkItem server** (`WorkItem.PullRequestsJson`, [ADR-0001](docs/adr/0001-standalone-workitem-server.md)'s source of truth) and not in the ILD-local database. That is what makes it survive the run going terminal, the `WorktreeRetentionSweeper` deleting the run row, and the ILD instance being reset or replaced; it is dropped only when the WorkItem itself is deleted.

Every path that learns something about a PR reports it to the server through `IWorkItemManager.RecordPullRequestAsync` — the PR node on creation (`LoopEngine`), the webhook/heartbeat on merge (`PrSyncService`), and a human via `LinkPullRequestAsync` — which is idempotent on the URL, so re-reporting updates in place. Because a report can be lost (crash, unreachable server, a run that predates this), `WorkItemManager` also re-reports on read whatever a live `LoopRun` still carries, so a PR is at most one read away from being durable again. The view unions the server's record with the live runs' PRs: only a live run holds a badge `Status` (from its `PrSnapshot`) and a `RunId` still worth linking to, and both simply fall away when the run is reclaimed. Distinct from `prUrl`, which keeps its narrower meaning — the **current** run's PR, and therefore empty once that run goes terminal (`/api/v1` is add-only per [ADR-0002](docs/adr/0002-manual-api-versioning.md), so `prUrl` was left alone rather than widened).
_Avoid_: PR list, linked PRs, past PRs

**Active Work Item Set**:
The work items this ILD instance is currently working on: one per local `LoopRun` still alive — `Running`, or `WaitingHuman`, which covers both a run parked at a Human/PR gate and one halted mid-node by a human (`LoopRun.IsHalted`). `RemoteWorkItemCoordinator` **derives** it from `ILoopRunStore.GetActiveWorkItemIdsAsync()` at the top of every poll pass — it is never maintained incrementally, so a run that ended releases its work item whatever status the item itself landed in. That store query is the single definition of "alive"; it is projected and untracked, so reading the set cannot leave a stale snapshot behind for a later re-read in the same scope (see **Run Refresh**). Membership ends when a run's row leaves that query — whoever wrote it, and there is deliberately no choke point. The engine's own lifecycle ends its runs directly (Completed at a Cleanup node, Failed on a node failure or crash); `StuckRunWatchdog` marks a driverless "Completed yet Running" row Failed, keeping the completion timestamp it was finalized with; `WorkItemManager` writes Completed on the Cleanup→Backlog path and Cancelled as its no-engine fallback; and `ILoopEngine.StopRunAsync` is how a run gets ended by the cancel button, by a human sending the item to Done, by a poll pass reacting to a server-side Done, and by startup reconcile. Each records the outcome it means to, and none has to remember to release a slot — which is the whole reason the set is derived rather than maintained. `StopRunAsync` ends the run and nothing else, returning the work item id so the caller can apply a disposition of its own — each wants a different one: Done for a human finishing it, HumanFeedback for `CancelRunAsync` behind the cancel button, and nothing at all for a poll pass reacting to a status the server already holds. So a human sending a work item to Done finishes the run parked behind it, and a poll pass that finds an item the server has marked Done closes whatever local run is still open for it. The same set does both jobs: it is the heartbeat list the server's stale reclaimer keys off, and its size is the concurrency gate against `scheduler.maxConcurrent` (claims made during a pass are counted locally on top, so a run that both starts and ends within one pass cannot hand its slot to a second claim in that pass). The cap is therefore **per instance** — deriving it from the server's `Running` status instead would silently make it global across every instance pointed at the same board. A pass that has Ready work but no room reports it as blocked along with the ids holding the slots; `WorkItemScheduler` logs that at Information on entering the state and on every change of holders, and at Debug on the passes in between, since being at the cap persists across passes and grace polling would otherwise repeat it every 5s.
...
**Chat Context**:
The ambient snapshot of what the user currently has open in the UI — the open WorkItem (and, when it has an active run, that run's worktree path) and/or the open Loop Editor's loop — attached to **each chat message** so the agent can act on whatever the human is looking at when they send it. What travels per message is the state that actually changes — the open ids, the worktree path, whether a Loop Editor is open — so it survives the user navigating between work items and the editor. The constant half is not per-message: a turn resumes the provider's agent session rather than replaying ILD's transcript, so every copy the preamble sends stays in the agent's history, and constant text prepended per turn costs the turn count **squared**. Such text is therefore a **Session Briefing** — delivered once into the agent session that will hold it, recorded per session so a rebound or forked one is briefed afresh, and pullable on demand for the rest of that session. The loop authoring guide is the only briefing today (`get_loop_authoring_guide`); a per-turn size budget in the tests is what keeps the next one from being inlined instead. It informs the agent of the open work item's worktree **path** (added as an allowed directory) rather than relocating where the agent runs — the Chat Session's working directory stays its scratch directory.
_Avoid_: chat scope, session context, focus

### Network

**Egress Proxy**:
The forward proxy the orchestrator runs on loopback (`EgressProxy`, `ILD_NETWORK_PROXY_PORT`, 3128 in the image) that every agent launch — CLI adapters, the interactive provider terminal, Worktree Previews — is pointed at through `HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY` set in `AgentIsolation.Route`/`RouteCommand`, the same seam that crosses to the agent uid. It learns the destination from the `CONNECT` target, the `Host` header or the TLS ClientHello SNI, never decrypts, records every destination to the **Network Log**, and answers `403` (or a TLS `access_denied` alert) when the **Network Policy** says no. It keeps the mode and lists cached for one second and is invalidated by every edit, so a change is judged on the agent's next connection; open tunnels are re-judged on every change and a now-blocked one is reset. Loopback is on `NO_PROXY` so the MCP callback to ILD's own API stays direct. Orchestrator-side spawns are not proxied. See [ADR-0019](docs/adr/0019-agent-egress-through-in-container-proxy.md).
_Avoid_: egress gateway, network sandbox, firewall (that is the enforcement below)

**Network Policy**:
The `network.mode` app setting (`off` / `whitelist` / `blacklist`, default `off`) plus the `NetworkPolicyEntries` rows. An entry is a host pattern on one list, either global or scoped to one AI provider: an exact host (`api.example.com`) or a leading-dot suffix (`.example.com`, also written `*.example.com`) that matches the domain and every subdomain; matching is case-insensitive, ignores a trailing dot, and IP literals match exactly. `whitelist` allows a connection iff an applicable whitelist entry matches; `blacklist` blocks iff an applicable blacklist entry matches; `off` consults nothing. A provider-scoped entry applies only to launches made for that provider (the proxy URL carries the provider as `Proxy-Authorization`); a connection with no provider is judged by global entries alone. That scope is attribution, not authentication — the credentials sit in the agent's own environment — so it tidies lists per tool rather than separating agents; a rule that must hold against a hostile agent belongs on the global lists. Entries of the list the mode is not using are ignored. Edited through `/api/v1/network/entries` and the **Network** section of Settings; every edit invalidates the proxy and is broadcast as `NetworkPolicyChanged`. Per-run and per-repository lists are deliberately not here — all runs share one agent uid, so the proxy cannot attribute a connection to a run.
...
- A **Worktree** (and its branch) is created per **LoopRun** by the Start node and kept after the run finishes for inspection; the `WorktreeRetentionSweeper` reclaims worktree, branch, and run once terminal longer than `run.retentionDays` (settings-driven; `0` disables; runs marked `Retain` are never reclaimed), and a **Run Cleanup** reclaims worktree and branch on demand without the run. The run's metrics fold into the analytics rollup as the row goes; its PR is already on the work item (see **PR History**), which is why nothing about reclaiming a run can lose it. See [ADR-0008](docs/adr/0008-worktree-and-branch-per-run.md)
...
- Failed/cancelled WorkItems: "Done" finishes the current run and discards; "Backlog" fully resets for re-planning. Neither destroys the run's worktree or branch — those are reclaimed by run deletion (manual or retention) or by a **Run Cleanup**, which frees them while keeping the run
...
- Auth is a real ASP.NET Core authentication scheme (`ILD.Api/Authentication/`), not a middleware that guesses from the path. `IldAuthenticationHandler` turns a bearer token into a principal — the agent service token becomes the `agent` role, a live session token becomes the `user` role named after its owner — reading it from `Authorization: Bearer`, a bare `Authorization` value, or an `?access_token=` query parameter (WebSocket and SignalR handshakes cannot set headers). Rejections keep the `{"error":"Unauthorized","message":"..."}` 401 body.
- Authorization is **deny-by-default and per-endpoint**: the fallback policy requires an authenticated caller in the `user` role, so any endpoint that says nothing is user-only and an agent token gets a 403 rather than being quietly admitted. The opt-outs are the whole list of exceptions: `[AllowAnonymous]` on `AuthController.Login`, `HealthController`, `LoggingController`, `MetricsController` and the SPA fallback (`MapFallbackToFile(...).AllowAnonymous()`); the `AgentOrUser` policy — any authenticated caller, since those are the only two roles — on `AgentController` (the entire agent surface) and on `WebhooksController`, whose external callers replay an operator-configured token that may be either principal's. Static assets are served before `UseAuthorization` and so stay anonymous, as the SPA shell needs. Webhook callers must additionally pass `IRemoteGitProviderAdapter.VerifyWebhookSignature` (the relevant adapter owns the check) on top of bearer auth. Forgejo and GitHub verify an HMAC signature; Azure DevOps signs nothing, so its adapter verifies the Basic credential its service-hook subscription was registered with. That credential occupies the Authorization header, so a `Basic` value is never read as an ILD token and such a caller carries its token in the `access_token` query instead.
