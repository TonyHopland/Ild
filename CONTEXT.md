...
- A **Worktree** (and its branch) is created per **LoopRun** by the Start node and kept after the run finishes for inspection; the `WorktreeRetentionSweeper` reclaims worktree, branch, and run once terminal longer than `run.retentionDays` (settings-driven; `0` disables; runs marked `Retain` are never reclaimed). The run's metrics fold into the analytics rollup as the row goes; its PR is already on the work item (see **PR History**), which is why nothing about reclaiming a run can lose it. See [ADR-0008](docs/adr/0008-worktree-and-branch-per-run.md)
