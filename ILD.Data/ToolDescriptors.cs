...
public static class ToolDescriptors
{
    ...
    public static readonly ToolDescriptor PullBranch = new()
    {
        ...
        Label = "Pull Branch",
        Description = "Pull the latest changes from origin into this work item's run branch — fetches with ILD's repository credentials and rebases the branch onto origin/<branch>. Use it to pick up commits pushed after the run started; git in the worktree has no credentials. The fetch covers every branch on the remote, not just yours, so afterwards you can read any of them locally with plain git — origin/<default branch> included, which is how you check whether your branch has fallen behind it and merge it in yourself if it has. What this tool will not do for you is that merge: it only rebases your branch onto its own remote counterpart. Returns an outcome of Updated, AlreadyUpToDate, NoRemoteBranch, DirtyWorktree (commit first; 'files' lists them), Conflict (rebase aborted, branch untouched; 'files' lists the conflicts to resolve) or RebaseRefused (git would not rebase at all — nothing to resolve, read 'message').",
