...
public static class LoopAuthoringGuide
{
    public const string Text =
        ...
        - Cmd: runs a shell command in the worktree, succeeds on exit 0. config.command. It runs as the agent user, with the agent's environment, HOME and network (egress) rules, and cannot read ILD's own secrets; a command that needs credentials must be given its own.
        ...
        - PR: opens/maintains a pull request. config.prDescriptionTemplate, config.customEdges. config.prCommentTemplate is NO LONGER POSTED — it used to go out on every re-visit and announced a second time what the round had already answered on the threads; an agent calls comment_on_pr when the round has something general to say, and a round with nothing to add leaves the pull request quiet. A template that still sets the field keeps working; the field does nothing. Reserved PR edges: on_rejected, on_merge_conflict, on_ci_failed, on_comment, on_approved, on_ci_passed, on_merged, on_abandoned — wire on_merged/on_abandoned to reach a terminal path. on_comment fires on review/comment items the run has not been handed yet — inline comments, comments on the pull request itself, and each review's own body, which is where most of a human review actually says anything — batched into one firing; while a review has changes requested on_rejected outranks it, and that round reads the comments with get_pr_review instead. Agents never write to the PR themselves: get_pr_review reads, while reply/resolve/comment_on_pr/close_pr_review_item only QUEUE an intent that the PR node sends at the end of the round, where a human can drop it first — so a loop that answers reviews must route back through its PR node or nothing goes out. close_pr_review_item records that a round read an item and chose not to answer it, optionally resolving its thread; it suppresses nothing, since a reviewer restating the point against new code still fires. By provider: GitHub and Azure DevOps read the review, reply and resolve; Forgejo reads and replies but cannot resolve a thread (no API), and the findings a Copilot review hides in its body exist on GitHub only. Nothing fails where a provider cannot do something — the tool answers with a message.
