...
public static class LoopAuthoringGuide
{
    public const string Text =
        ...
        - PR: opens/maintains a pull request. config.prDescriptionTemplate, config.prCommentTemplate, config.customEdges. Reserved PR edges: on_rejected, on_merge_conflict, on_ci_failed, on_comment, on_approved, on_ci_passed, on_merged, on_abandoned — wire on_merged/on_abandoned to reach a terminal path. on_comment fires on review/comment items the run has not been handed yet — inline comments, comments on the pull request itself, and each review's own body, which is where most of a human review actually says anything — batched into one firing; while a review has changes requested on_rejected outranks it, and that round reads the comments with get_pr_review instead. Agents never write to the PR themselves: get_pr_review reads, while reply/resolve only QUEUE an intent that the PR node sends at the end of the round, where a human can drop it first — so a loop that answers reviews must route back through its PR node or nothing goes out. By provider: GitHub and Azure DevOps read the review, reply and resolve; Forgejo reads and replies but cannot resolve a thread (no API), and the findings a Copilot review hides in its body exist on GitHub only. Nothing fails where a provider cannot do something — the tool answers with a message.
