...
public enum HaltReason
{
    ...
    /// setting, which waits out any reset time that notice stated, retries on
    /// the interval they configured (<c>throttle.retryDelayMinutes</c>, default
    /// 60) for as many attempts as they allowed (<c>throttle.maxRetries</c>,
    /// default 6), and then leaves the run parked here anyway.
    /// </summary>
    Throttled = 2,
