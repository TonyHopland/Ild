...
public class PreviewProxyBaseTests
{
    ...
    public void An_unset_value_disables_the_proxy_entirely()
    {
        ...
        Assert.False(proxyBase.TryGetHostLabel("wi-7.ild.example", out _));
    }

    [Theory]
    [InlineData("http://ild.example", "http", "ild.example", null)]
    ...
    [InlineData("  ild.example:8080  ", "http", "ild.example", 8080)]
    ...
    public void An_unusable_value_disables_the_proxy_and_says_why(string value)
    {
        ...
        Assert.Contains("http://ild.example", proxyBase.ConfigurationError);
    }

    [Theory]
    [InlineData("wi-7.ild.example", "wi-7")]
    [InlineData("wi-7-api.ild.example", "wi-7-api")]
    [InlineData("WI-7.ILD.EXAMPLE", "WI-7")]
    public void A_subdomain_of_the_base_yields_its_label(string requestHost, string expected)
    {
        var proxyBase = PreviewProxyBase.Parse("http://ild.example");
    ...
    [Theory]
    [InlineData("ild.example")]        // the apex host: this is the ILD UI, never a preview
    [InlineData(".ild.example")]       // empty label
    [InlineData("notild.example")]     // suffix match without the dot separator
    [InlineData("ild.example.attacker.test")]
    ...
    [Theory]
    [InlineData("http://ild.example", "http://wi-7.ild.example")]
