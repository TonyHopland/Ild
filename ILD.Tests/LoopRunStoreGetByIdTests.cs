...
public class LoopRunStoreGetByIdTests
{
    ...
    public async Task GetRunNodesAsync_returns_nodes_ordered_by_CreatedAt_ascending()
    {
        ...
        db.Context.RemoteProviders.Add(remote);
        db.Context.Repositories.Add(repo);
        var wi = new WorkItem { Id = Guid.NewGuid(), Title = "test", Status = WorkItemStatus.Running, RepositoryId = repo.Id };
        db.Context.WorkItems.Add(wi);
        ...
        var run = new LoopRun { Id = Guid.NewGuid(), WorkItemId = wi.Id, LoopTemplateVersionId = ltv.Id, Status = LoopRunStatus.Running, RecoveryPolicy = RecoveryPolicy.AutoResume };
